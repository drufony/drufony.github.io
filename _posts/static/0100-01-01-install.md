...
## Enable Symfony integration module

You will need to enable the [Symfony module](https://github.com/bangpound/symfony-module) which was downloaded when you ran `composer create-project`. Without this module, you will not be able to log out of Drupal without deleting the session cookie.

This command will  enable the module, but it also produce fatal errors with the message "Call to a member function log() on a non-object." **This error is harmless. The command succeeds.** There is an [open issue to resolve this](https://github.com/bangpound/symfony-drupal/issues/9).
